...
exports.createFile = catchAsync(async (req, res) => {
  ...
  if (stat) throw new AppError(`There is already a file with name '${name}' in this folder.`, 400)
...
exports.renameFile = catchAsync(async (req, res) => {
  ...
  if (!newName) throw new AppError('Please provide a valid new file name.', 400)
  ...
  if (stat) throw new AppError(`A file with name '${newName}' already exists.`, 400);
...
exports.getFileContent = catchAsync(async (req, res) => {
  ...
  if (!path) throw new AppError('Please provide a valid path.', 400);
  if (!name) throw new AppError('Please provide a file name to read content.', 400);
  ...
  const stat = await readStat(PATH.join(path, name));
  if (!stat) throw new AppError(`You don't have permission to read file '${name}'.`, 400);
  ...
  if (!supportedFiles.includes(extension))
    throw new AppError(`File with extension ${extension} do not support to view or edit.`, 400)
...
exports.updateFileContent = catchAsync(async (req, res) => {
  ...
  if (!name) throw new AppError('Please provide file name to update content.', 400);  
  if (!content) throw new AppError('Please provide valid content to update.', 400);
  ...
  const stat = await readStat(PATH.join(path, name));
  if (!stat) throw new AppError(`You don't have permission to write file '${name}'.`, 400);  
...
exports.copyFile = catchAsync(async (req, res) => {
  ...
  if (!stat) throw new AppError(`You don't have permission to copy file '${name}'.`, 400);   

  // Check if there is already file with the same name in destination directory
  stat = await readStat(PATH.join(dest, name));
  if (stat) throw new AppError(`There is already file with name '${name}' in the destination path.`, 400)
